...
public class SensorsService extends Service {
            ...
            for (SensorInfo si : sensors) {
                if (si.isRegistered()) {
                    if (si.getLatestReading() != null)
                        dbAdapter.insertGenericLog(si.getDatabaseTag(),
                                si.getLatestReading());
                    else
                        Log.w(LOG_TAG,
                                "registered sensor requested null reading insertion - aborting | sensor: "
                                        + sManager.getDefaultSensor(
                                                si.getType()).getName());
                }
            }

            handler.postDelayed(this, (Long) AppProperties.timingProperties
    ...
    public void onCreate() {
        ...
        // handler posts logReadings runnable delayed in order to give sensors
        // time to broadcast first events
        handler.postDelayed(logReadings, (Long) AppProperties.timingProperties
                .get("collect-sensors-rate-ms"));
        ...
        for (SensorInfo si : sensors)
            if (si.isRegistered())
                sManager.unregisterListener(sel,
                        sManager.getDefaultSensor(si.getType()));
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (dbAdapter.isOpenWritable()) {
            ...
            case Sensor.TYPE_ACCELEROMETER:
                ...
                if (create) {
                    if (si == null) {
                        sManager.registerListener(sel, sManager
                                .getDefaultSensor(Sensor.TYPE_ACCELEROMETER),
                                SensorManager.SENSOR_DELAY_NORMAL);
                        sensors.add(new SensorInfo(Sensor.TYPE_ACCELEROMETER,
                                "Accelerometer"));
                    ...
                    if (si != null) {
                        sManager.unregisterListener(sel, sManager
                                .getDefaultSensor(Sensor.TYPE_ACCELEROMETER));
                ...
                if (create) {
                    if (si == null) {
                        sManager.registerListener(sel, sManager
                                ...
                                SensorManager.SENSOR_DELAY_NORMAL);
                        sensors.add(new SensorInfo(Sensor.TYPE_GYROSCOPE,
                                "Gyroscope"));
                    } else if (!si.isRegistered()) {
                        sManager.registerListener(sel, sManager
                                .getDefaultSensor(Sensor.TYPE_GYROSCOPE),
            ...
            case Sensor.TYPE_LIGHT:
                ...
                if (create) {
                    if (si == null) {
                        sManager.registerListener(sel,
                                sManager.getDefaultSensor(Sensor.TYPE_LIGHT),
                                SensorManager.SENSOR_DELAY_NORMAL);
                ...
                if (create) {
                    if (si == null) {
                        sManager.registerListener(sel, sManager
                                ...
                                SensorManager.SENSOR_DELAY_NORMAL);
                        sensors.add(new SensorInfo(Sensor.TYPE_PROXIMITY,
                                "Proximity"));
                    } else if (!si.isRegistered()) {
                        sManager.registerListener(sel, sManager
                                .getDefaultSensor(Sensor.TYPE_PROXIMITY),
